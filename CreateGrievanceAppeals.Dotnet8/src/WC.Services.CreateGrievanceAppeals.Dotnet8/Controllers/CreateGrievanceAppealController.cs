...
using System.Net;
using WC.Services.CreateGrievanceAppeals.Dotnet8.Models.Data;
using WC.Services.CreateGrievanceAppeals.Dotnet8.Models;

namespace WC.Services.CreateGrievanceAppeals.Dotnet8.Controllers
{
    using System;
    ...
    public class CreateGrievanceAppealController : Controller
    {
        private readonly log4net.ILog _logger;
      
        private readonly GrievanceAppealManager _grievanceAppealManager;
        private readonly IConfiguration _configuration;
        public CreateGrievanceAppealController(log4net.ILog logger, IConfiguration configuration, GrievanceAppealManager grievanceAppealManager)
        {
            _logger = logger;
        ...
        [HttpPost]
        public IActionResult Post([FromBody]RequestPayload<GrievanceAppealSubmission> request)
        {
            Console.WriteLine("CreateGrievanceAppealController.Post");
            if(request == null)
                Console.WriteLine("request is null");
            if(request.Payload == null)
                Console.WriteLine("request.Payload is null");
            try
            {
                _logger.Info(JsonSerializer.Serialize(request));
                ...
                if (!ModelState.IsValid)
                {
                    ...
                    {
                        _logger.Info($"{loggerInit} {error}");
                    }

                    _logger.Info($"{loggerInit} Service Aborted");
                ...
                if (grievanceAppealsData.Message != "SUCCESS")
                {
                    ...
                    return BadRequest(grievanceAppealsData.Message);
                }

                _logger.Info($"{loggerInit} Getting appication settings.");
               // var onBaseSettings = _configuration.GetSection("OnBaseSettings").GetChildren();
                // Determine environment
                OnBaseConnectionParameters connParams = new OnBaseConnectionParameters
                {
                    Datasource = _configuration["OnBaseSettings:OnBase.Datasource"],
                    ServerURL = _configuration["OnBaseSettings:OnBase.Url"],
                    UserName = _configuration["OnBaseSettings:OnBase.UserName"],
                    Password = _configuration["OnBaseSettings:OnBase.Password"]
                };

                _logger.Info($"{loggerInit} Retrieved appication settings.");

                GrievanceAppealsResult results = _grievanceAppealManager.CreateGrievanceAppeals(
                    grievanceAppealsData,
                    connParams);

                return (results.Status == "FAILURE")? BadRequest(results) : Ok(results);
            }
            catch (Exception e)
            {
        ...
        private static GrievanceAppealsData FormatGrievanceAppeals(
            ...
            string loggerInit, log4net.ILog _logger)
        {
            ...
            catch (Exception ex)
            {
                ...
                _logger.Error($"{loggerInit} Error in request data: {ex}");
                throw;
            }
            ...
            string loggerInit, log4net.ILog _logger)
        {
            ...
            string erMessage =
                ...
                " limit.";
            _logger.Error($"{loggerInit} {erMessage}");
            return erMessage;
