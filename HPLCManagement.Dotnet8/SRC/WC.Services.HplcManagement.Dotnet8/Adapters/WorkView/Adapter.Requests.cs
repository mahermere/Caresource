...
namespace WC.Services.HplcManagement.Dotnet8.Adapters.WorkView
{
	...
	using Attribute = WC.Services.HplcManagement.Dotnet8.Models.Attribute;
	...
	public partial class Adapter
	{
		...
		/// </returns>
		public long CreateRequest(Data request)
		{
			...
			Data wvo)
		{
			...
			List<Attribute> attrs = wvo.Properties.Select(
		...
		/// <returns></returns>
		public Data GetObject(long objectId)
		{
			Class requestClass = _workViewApplication.Classes.Find(Constants.Request.ClassName);

			Object request = requestClass.GetObjectByID(objectId);

			return _mapper.GetMappedModel(request);
		}
		...
		/// <returns></returns>
		public IEnumerable<Data> HieSearch(
			string className,
			string filterName,
			string[] attributes,
