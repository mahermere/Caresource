...
namespace WC.Services.HplcManagement.Dotnet8.Adapters.WorkView
{
	...
	public partial class Adapter
	{
		...
		public IEnumerable<string> GetDataSet(
			...
			string className)
		{
			_logger.Debug($"Application Name {_applicationName}", null);
			...
			IEnumerable<string> items = ExecuteSqlDatasetSearch(
				dataSetName,
				wvClass);
			...
			Class wvClass)
		{
            ...
            using (IDbConnection db = new SqlConnection(_configuration["OnBaseSettings:OnBase.ConnectionString"]))
            {
                using (IDbCommand dbCommand = db.CreateCommand())
                {
					...
					{
						items.Add(
                            dataReader.GetString(dataReader.GetOrdinal("Available Values")).Trim());
					}
				}
			}
		...
		private static IEnumerable<string> ExecuteSql(string sql)
		{
			...
			int timeout = Convert.ToInt32(_configuration["OnBaseSettings:OnBase.Connection.Timeout"]);
