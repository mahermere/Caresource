...
namespace WC.Services.Document.Dotnet8.Adapters.v6
{
    ...
    public partial class OnBaseSqlAdapter : IOnBaseSqlAdapter<Models.v6.DocumentHeader>
	{
		...
		private int Timeout => Convert.ToInt32(
			_configuration["OnBaseSettings:OnBase.Connection.Timeout"]);
			...
			string GetWorkViewClaimsFrom()
			{
				...
				IEnumerable<Class> classes =
					_onBaseAdapter.GetWorkViewClasses(
						_configuration["WorkViewSettings:WorkView.Application"]);
		...
		private IEnumerable<string> LetterTypeValues(
			...
			bool providerFacing)
		{
			string docAdminConsoleName = _configuration["WorkViewSettings:WorkView.DocumentAdminConsole.Name"];
			string docAdminConsoleClass = _configuration["WorkViewSettings:WorkView.DocumentAdminConsole.Class"];
			IEnumerable<Class> wvclasses = _onBaseAdapter.GetWorkViewClasses(docAdminConsoleName);
			Class wvClass = wvclasses.FirstOrDefault(c => c.Name == docAdminConsoleClass);
...

			{
                ...
                //	.ToList();
                db.Open();
                using (SqlCommand command = new SqlCommand(sql, (SqlConnection)db))
                {
                    using (SqlDataReader reader = command.ExecuteReader())
                    {
                        ...
                        while (reader.Read())
                        {
                            letterTypes.Add(reader.GetString(0).Trim());
                        }
                        return letterTypes;
                    }
                }
