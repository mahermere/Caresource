...
namespace WC.Services.Document.Dotnet8.Adapters.v6
{
    ...
    public class SqlAdapter : ISqlAdapter
	{
		...
		public SqlAdapter(log4net.ILog logger,IOnBaseAdapter onBaseAdapter,
			...
			IEnumerable<string> displayColumns)
		{
			...
			int totalRecords = 0;
            using (var db = new SqlConnection(_configuration["OnBaseSettings:OnBase.ConnectionString"]))
            {
                db.Open();
                        ...
                        while (reader.Read())
                        {
                            var document = new DocumentHeader
                            {
                                ...
                                DocumentType = FindProperCaseDocType(
                                    reader.GetString(reader.GetOrdinal("DocumentType")).Trim()),
		...
		private IEnumerable<string> LetterTypeValues()
		{
			Class wvClass = _onBaseAdapter.GetWorkViewClasses(
					_configuration["WorkViewSettings:WorkView.DocumentAdminConsole.Name"])
				.FirstOrDefault(c =>
					c.Name == _configuration["WorkViewSettings:WorkView.DocumentAdminConsole.Class"]);
			...
			using (var db = new SqlConnection(
						_configuration["OnBaseSettingsOnBase.ConnectionString"]))
			{
                db.Open();
                ...
                using (var cmd = db.CreateCommand())
                {
                    ...
                    using (var reader = cmd.ExecuteReader())
                    {
                        ...
                        {
                            letterTypes.Add(
                                reader.GetString(reader.GetOrdinal("LetterType"))
                                    .Trim());
                        }
