...
namespace WC.Services.Document.Dotnet8.Adapters
{
    ...
    public class OnBaseGetDocumentAdapter : IGetDocumentAdapter<OnBaseDocument>
	{
		...
		{
			Document document = _repo.Application.Core.GetDocumentByID(
				documentId,
			...
			if (_fileTypes.ContainsKey(ext))
			{
							...
							StringComparison.InvariantCulture))
				{
					...
					ImageDataProvider imageProvider =
						_repo.Application.Core.Retrieval.Image;

					return imageProvider.GetDocument(document.DefaultRenditionOfLatestRevision);
				}
			}

			NativeDataProvider provider = _repo.Application.Core.Retrieval.Native;
			...
			IListDocumentsRequest request)
		{
			...
			DocumentQuery documentQuery = _repo.Application.Core.CreateDocumentQuery();
