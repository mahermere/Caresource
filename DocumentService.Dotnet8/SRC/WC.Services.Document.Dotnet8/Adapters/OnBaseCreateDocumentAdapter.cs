namespace WC.Services.Document.Dotnet8.Adapters
{
    ...
    public class OnBaseCreateDocumentAdapter : ICreateDocumentAdapter
	{
			...
			List<Stream> documentPages)
		{
			...
			{
				throw new ArgumentException($"Could not find document type '{documentType}' in OnBase.");
			}
			...
			FileType fileType =
				GetFileTypeByExtension(fileTypeExtension) ?? obDocumentType.DefaultFileType;
