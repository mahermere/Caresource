...
namespace CareSource.WC.Core.Http
{
	...
	public class RestClient : IRestClient
	{
		...
		{
			_logger.LogInformation($"Sending DELETE request to endpoint '{endPoint}{action}'.");

			BuildClient(userName, password);

			SetRequestHeaders(_client.DefaultRequestHeaders, contentType);
		...
		public (HttpStatusCode StatusCode, string Response, TResponseModel Model) Get<TRequestModel,
			...
			TRequestModel requestData)
		{
			_logger.LogInformation($"Sending GET request to endpoint '{endPoint}{action}'.");
		...
		public (HttpStatusCode StatusCode, string Response, TResponseModel Model) Patch<TRequestModel,
			...
			TRequestModel requestData)
		{
			_logger.LogInformation($"Sending PATCH request to endpoint '{endPoint}{action}'.");

			BuildClient(userName, password);

			StringContent content = BuildBody(requestData, contentType);
		...
		public (HttpStatusCode StatusCode, string Response, TResponseModel Model) Post<TRequestModel,
			...
			TRequestModel requestData)
		{
			_logger.LogInformation($"Sending POST request to endpoint '{endPoint}{action}'.");
		...
		public (HttpStatusCode StatusCode, string Response, TResponseModel Model) Put<TRequestModel,
			...
			TRequestModel requestData)
		{
			_logger.LogInformation($"Sending PUT request to endpoint '{endPoint}{action}'.");
			...
			string password)
		{
			...
			if (username == null
				&& password == null)
			{
				username = _transactionContextManager.CurrentContext?.SecurityContext?.AuthenticatedUserId;
				password = _transactionContextManager.CurrentContext?.SecurityContext?.Password;

				_logger.LogInformation("Pulling username and password from TransactionContext.");
			}
				...
				&& !password.IsNullOrWhiteSpace())
			{
				...
				_logger.LogDebug($"Adding Username '{username}' and Password '{password}' to Authorization header.");
			}
		}
			...
			HttpResponseMessage response)
		{
			...
			TResponseModel model = DeserializeResponse<TResponseModel>(results, contentType);
		...
		private void SetRequestHeaders(
			...
			string contentType)
		{
			_logger.LogInformation("Start Set Request Headers", contentType, headers);
			...
			headers.Add("X-Transaction-Context", encodedTc.Base64Encode());
			headers.Add("X-Transaction-Context-Content-Type", MediaTypes.ApplicationJson);
