...
namespace ImportProcessor.Managers.v1
{
	using System;
	...
	public class ImportProcessorManager : IImportProcessorManager<ImportProcessorResponse>
	{
			...
			Guid correlationGuid)
		{
			...
			{
				case RNL:
					cpsDirectory = _settingsAdapter.GetSetting("CPSDirectoryLocation");
					break;

				case CBRL:
					cpsDirectory = _settingsAdapter.GetSetting("CBRLDirectoryLocation");
					break;

				case EOB:
					cpsDirectory = _settingsAdapter.GetSetting("EOBDirectoryLocation");
					break;

				case COBRNL:
					cpsDirectory = _settingsAdapter.GetSetting("COBRNLDirectoryLocation");
					break;
			}
			...
			if (Directory.GetFiles(processingLocation)
				...
				0)
			{
				_logger.LogInformation("Claims Service is currently busy processing a batch.");
			...
			foreach (string zipFileName in zipFileNames)
			{
				...
				_logger.LogDebug($"Backing up zip file: {zipFileName}");
				CopyToBackupFolder(
					zipFileName,
					cpsDirectory);
				...
				{
					_logger.LogDebug($"Deleting Existing Folder: {destinationFolder}");
				...
				_logger.LogDebug($"Extracting file to: {destinationFolder}");
				...
				if (jsonFileCount != 1)
				{
					...
					_logger.LogError($"Json file missing in zip file:{zipFileName}");
				...
				_logger.LogDebug($"Deserializing Json file: {jsonFileName}");
				...
				if (cpsImportData != null)
				{
					...
					_logger.LogDebug(
						$"Total Number of Documents (JSON): {jsonDocsCount}; (PDF): {pdfFileCount}.");
				...
				else
				{
					...
					_logger.LogError($"Error deserializing the JSON file :{jsonFileName}");
				...
				_logger.LogInformation($"Document Type Name: {docTypeName}");
				...
				if (documentTypeIndicator == EOB && updateKeywords)
				{
					try
					{
						...
						do
						{
							...
							{
								_logger.LogError("Error in Document/Search call.");
						...
						if (totalFailures.Any())
						{
							...
							_logger.LogError(
								"Error Updating Document Keywords.");
						}
						else
						{
							CleanupFolders(
				...
				foreach (Document document in cpsImportData.documents)
				{
					...
					try
					{
						...
						if (documentId <= 0)
						{
							...
							_logger.LogError(
								"Error creating OnBase document.");
					...
					try
					{
						...
						if (documentTypeIndicator != DoctypeIndEob)
						{
							...
							_logger.LogDebug($"Letter Number being processed: {letterNumber}");
							...
							if (letterNumber.IsNullOrWhiteSpace() ||
								providerId.IsNullOrWhiteSpace())
							{
								_logger.LogError(
					...
					catch (Exception e)
					{
						...
						_logger.LogError(e,
							"An unexpected error occurred creating the WV Document object.");
					...
					try
					{
						IEnumerable<Claim> errorsClaims = document.document.claims.Where(
							c => c.attributes.Any(i => i.Value.IsNullOrWhiteSpace()));
					...
					catch (Exception e)
					{
						...
						_logger.LogError(e,
							"An unexpected error occurred when creating the WV claims object.");
